...
function buildUserPrompt({ transcript, options, videoUrl, title }) {
  const parts = [];
...
async function analyzeVideo({ transcript, segments, options, videoUrl, title }) {
  if (!transcript || transcript.length < 10) {
    return { ok: false, error: 'Transcripción vacía o demasiado corta' };
  }
  ...
  try {
    ...
    const response = await client.messages.create({
      ...
      messages: [
        { role: 'user', content: buildUserPrompt({ transcript, options, videoUrl, title }) },
      ],
    });

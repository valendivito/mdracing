...
/* ════════════════════════════════════════════════════════════
 * CART SESSIONS — recuperación de carritos abandonados (Sprint F)
 * ════════════════════════════════════════════════════════════ */

/**
 * Upsert de un cart_session (carrito o checkout en progreso).
 * Identifica por customer_email (case-insensitive). Si ya existe un
 * registro pendiente para ese email, actualiza items/total/updated_at
 * en lugar de crear uno nuevo.
 *
 * IMPORTANTE: Solo upserta si NO hay un converted_order_id ya seteado
 * para ese email reciente (evita resucitar carritos ya completados).
 */
async function saveCartSession({ email, name, phone, items, total }) {
  try {
    const db = getDB();
    const normalizedEmail = String(email).trim().toLowerCase();
    if (!normalizedEmail) return { ok: false, error: 'email requerido' };

    // Buscar el cart pendiente más reciente de este email (últimos 7 días)
    ...
    if (existing && existing.length > 0) {
      ...
      const { data, error } = await db
        ...
        .single();
      if (error) return { ok: false, error: error.message };
      return { ok: true, cart: data, created: false };
    } else {
      // Crear nuevo
      const { data, error } = await db
        .from('cart_sessions')
        ...
        .single();
      if (error) return { ok: false, error: error.message };
      return { ok: true, cart: data, created: true };
    }
  } catch (e) {
    console.error('[db.saveCartSession] exception:', e);
    return { ok: false, error: e.message };
  }
}

/**
 * Marca como convertido cualquier cart_session pendiente del email dado.
 * Llamar después de guardar una order exitosa.
 */
async function markCartConverted(email, orderId) {
  try {
    ...
    const { error } = await db
      .from('cart_sessions')
      .update({ converted_order_id: orderId, updated_at: new Date().toISOString() })
      .ilike('customer_email', normalizedEmail)
      .is('converted_order_id', null);
    if (error) {
      console.error('[db.markCartConverted] error:', error);
      return { ok: false, error: error.message };
    }
    return { ok: true };
  } catch (e) {
    console.error('[db.markCartConverted] exception:', e);
    return { ok: false, error: e.message };
  }
}

/**
 * Busca cart_sessions pendientes que necesitan recordatorio.
 *
 * @param {object} opts
 * @param {number} opts.hoursAgo - minimo horas desde updated_at (4 o 24)
 * @param {'reminder_4h_sent_at'|'reminder_24h_sent_at'} opts.reminderField
 */
async function findCartsForReminder({ hoursAgo, reminderField }) {
  try {
    const db = getDB();
    const threshold = new Date(Date.now() - hoursAgo * 60 * 60 * 1000).toISOString();
    // Solo carts no convertidos, no notificados aún para este recordatorio,
    // con updated_at más antiguo que el threshold.
    // Limitamos a últimos 7 días para no resucitar carts viejísimos.
    const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
    const { data, error } = await db
      .from('cart_sessions')
      .select('*')
      ...
      .limit(50); // safety cap por corrida del cron
    if (error) {
      console.error('[db.findCartsForReminder] error:', error);
      return { ok: false, error: error.message };
    }
    return { ok: true, carts: data || [] };
  } catch (e) {
    console.error('[db.findCartsForReminder] exception:', e);
    return { ok: false, error: e.message };
  }
}

/**
 * Marca un cart_session como notificado (setea reminder_*_sent_at = now).
 */
async function markReminderSent(cartId, reminderField) {
  try {
    const db = getDB();
    const patch = { [reminderField]: new Date().toISOString() };
    const { error } = await db.from('cart_sessions').update(patch).eq('id', cartId);
    if (error) return { ok: false, error: error.message };
    return { ok: true };
  } catch (e) {
    return { ok: false, error: e.message };
...
/* ════════════════════════════════════════════════════════════
 * VIDEO ANALYZER — jobs de análisis de videos en background
 * ════════════════════════════════════════════════════════════ */

/**
 * Crea un job nuevo de análisis de video.
 * @param {object} params
 * @param {string} params.sourceUrl - URL del video (YouTube/TikTok/IG)
 * @param {string} params.title - Título opcional
 * @param {object} params.options - Qué outputs quiere el usuario (transcript, hooks, script, caption, hashtags)
 */
async function createVideoJob({ sourceUrl, title, options }) {
  try {
    const db = getDB();
    const { data, error } = await db
      .from('video_jobs')
...
/** Lista jobs ordenados por created_at DESC, con paginación simple. */
async function listVideoJobs({ limit = 50, offset = 0 } = {}) {
  try {
    const db = getDB();
    const cap = Math.min(limit, 200);
    const { data, error, count } = await db
      .from('video_jobs')
      .select('id, source_url, title, status, options, error_message, duration_sec, created_at, started_at, finished_at', { count: 'exact' })
      .order('created_at', { ascending: false })
      .range(offset, offset + cap - 1);
    if (error) return { ok: false, error: error.message };
    return { ok: true, jobs: data || [], total: count || 0 };
  } catch (e) {
    return { ok: false, error: e.message };
  }
}

/** Obtiene un job por id con todos los campos (incluye transcript + analysis). */
...
/** Actualiza un job (status, transcript, analysis, error). */
async function updateVideoJob(id, patch) {
  try {
    const db = getDB();
    const allowed = {};
    if (patch.status !== undefined) allowed.status = patch.status;
    if (patch.transcript !== undefined) allowed.transcript = patch.transcript;
    if (patch.analysis !== undefined) allowed.analysis = patch.analysis;
    if (patch.audio_url !== undefined) allowed.audio_url = patch.audio_url;
    if (patch.duration_sec !== undefined) allowed.duration_sec = patch.duration_sec;
    if (patch.error_message !== undefined) allowed.error_message = patch.error_message;
    if (patch.title !== undefined) allowed.title = patch.title;
    if (patch.started_at !== undefined) allowed.started_at = patch.started_at;
    if (patch.finished_at !== undefined) allowed.finished_at = patch.finished_at;
    if (patch.attempts !== undefined) allowed.attempts = patch.attempts;
    const { error } = await db.from('video_jobs').update(allowed).eq('id', id);
    if (error) return { ok: false, error: error.message };
    return { ok: true };
  } catch (e) {
    return { ok: false, error: e.message };
...
/**
 * Devuelve hasta N jobs en status='pending' o jobs 'processing' que llevan
 * más de 10 min sin actualizarse (recuperación de jobs que se colgaron).
 */
async function findPendingVideoJobs(limit = 3) {
  try {
    const db = getDB();
    ...
    const { data, error } = await db
      .from('video_jobs')
      .select('*')
      .or(`status.eq.pending,and(status.eq.processing,started_at.lt.${stuckThreshold})`)

...
function paymentMethodLabel(method) {
  return method === 'cash'
    ? 'Efectivo / Transferencia (coordina retiro o envío)'
    : 'Mercado Pago';
}
  ...
  if (isCash && isPickup) {
    nextStepsHtml = `
      <div style="background:#fff3d9;border-left:4px solid #d4a020;padding:14px 18px;border-radius:6px;margin:16px 0">
        ...
        <p style="margin:8px 0 0;font-size:14px">Te contactamos por WhatsApp cuando esté listo para retirar en <strong>${escapeHtml(order.shipping.label)}</strong>. Lo abonás en efectivo al retirarlo.</p>
      </div>`;
  } else if (isCash) {
    nextStepsHtml = `
      <div style="background:#fff3d9;border-left:4px solid #d4a020;padding:14px 18px;border-radius:6px;margin:16px 0">
        <p style="margin:0;font-weight:600;color:#9a7000">📍 Tu pedido quedó RESERVADO</p>
        <p style="margin:8px 0 0;font-size:14px">Te contactamos por WhatsApp para coordinar la forma de pago (efectivo o transferencia) y el envío.</p>
      </div>`;
  } else {
    nextStepsHtml = `

...
/**
 * Source A — Endpoint timedtext directo de YouTube (no requiere Innertube).
 *
 * YouTube expone los subtítulos en una URL pública del tipo:
 *   https://www.youtube.com/api/timedtext?v=<ID>&lang=es&fmt=srv3
 *
 * Esto funciona incluso cuando la API Innertube oculta los captionTracks
 * por anti-bot. Probamos varios idiomas + asr (auto-generados).
 */
async function fetchYouTubeTimedText(videoId) {
  const langCandidates = [
    { lang: 'es', kind: '' },         // español manual
    { lang: 'es-419', kind: '' },     // español latam
    { lang: 'es', kind: 'asr' },      // español auto-generado
    { lang: 'es-419', kind: 'asr' },
    { lang: 'en', kind: '' },         // fallback inglés
    { lang: 'en', kind: 'asr' },
  ];

  for (const { lang, kind } of langCandidates) {
    ...
    try {
      ...
      if (!xml || xml.length < 50) continue; // respuesta vacía → no hay
      const parsed = parseYouTubeTimedTextSrv3(xml);
...
/**
 * Parser del formato srv3 (XML JSON-like) que devuelve timedtext.
 * Es similar al XML clásico pero con estructura distinta:
 *   <p t="500" d="2300">...<s>texto</s>...</p>
 */
function parseYouTubeTimedTextSrv3(xml) {
  const segments = [];
  ...
  while ((m = pRe.exec(xml)) !== null) {
    ...
    const durMs = dMatch ? parseInt(dMatch[1], 10) : 0;
    // Concatenar texto interno (puede tener <s> con timestamps por palabra)
...
async function fetchYouTubeCaptions(videoId) {
  // ── PASO 1: intentar timedtext directo (más confiable, sin Innertube) ──
  const directCaptions = await fetchYouTubeTimedText(videoId);

  // Metadata desde Innertube (siempre tratamos de tenerla, sirve para
  // duración, descripción del autor, etc.)
  const clients = [
    {
      name: 'WEB',
  ...
  // ── PRIORIDAD 1: si timedtext directo trajo captions, usar eso ──
  if (directCaptions) {
    return {
      transcript: directCaptions.transcript,
      segments: directCaptions.segments,
      duration: lastDuration,
      language: directCaptions.language,
      title: lastTitle,
      description: lastDescription,
      isAuto: directCaptions.isAuto,
      hasCaptions: true,
      source: 'timedtext',
      triedClients,
    };
  ...
  // ── PRIORIDAD 2: si Innertube devolvió tracks, intentar bajarlos ──
  // (fallback histórico, ahora YouTube lo bloquea en mayoría de casos)
  if (!foundTracks) {
    return {
      transcript: '',
      segments: [],
